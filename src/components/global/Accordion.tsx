...
const Accordion = ({ name, selected, headerTextComp, setSelected, children }: TAccordion) => {
	return (
		<section className='py-2'>
			<div role='button' onClick={() => setSelected(name === selected ? '' : name)} className=" h-16 flex gap-4 justify-between
		items-center py-3 pb-0">
				{headerTextComp || <H5 className='font-semibold !text-lg text-textHeader'  >{name}</H5>}
				{selected === name ? (
